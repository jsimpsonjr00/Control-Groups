(function ( $ ) {
	...
	$.JSUI.ControlGroup = function ( $group, childSelector, firstActive, parentLock ) {
		this.$active 		= null;
		this.bLocked 		= false;
	...
	$.JSUI.ControlGroup.prototype = {
		constructor: $.JSUI.ControlGroup,
		...
		supportTransition: function () { //Support old browsers by manually triggering a transition end event, if needed
			if( !JSUI.support.transition && this.$active ) {
		...
		transitionEnd: function ( e ) {
			var self = (this instanceof JSUI.ControlGroup) ? this : e.data.self,
				$target = $(e.target);
	...
	$.fn.controlGroup = function( childSelector, firstActive, parentLock ) {
		var group;
		this.each( function () {
			group = new $.JSUI.ControlGroup( $(this), childSelector, firstActive, parentLock );
		});
